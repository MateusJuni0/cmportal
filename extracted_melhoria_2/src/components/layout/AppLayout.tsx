...
import { Brain, Sparkles, Bot, Terminal, Moon, Sun, LayoutTemplate, PanelsTopLeft, Box, GitCommit, Search, ShieldAlert, Zap, LineChart, Globe, Users, Vault } from "lucide-react";
...
const innovationItems = [
  { name: "Market Intel", path: "/innovation/market", icon: LineChart },
  { name: "Agent Warfare", path: "/innovation/warfare", icon: ShieldAlert },
  { name: "Growth Engines", path: "/innovation/growth", icon: Zap },
];

const nexusItems = [
  { name: "God-Eye Nexus", path: "/nexus/omniscience", icon: Globe },
  { name: "AI Boardroom", path: "/nexus/boardroom", icon: Users },
  { name: "Quantum Treasury", path: "/nexus/treasury", icon: Vault },
];
    ...
    <div className="flex h-screen w-full bg-zinc-50 dark:bg-black text-slate-900 dark:text-zinc-50 overflow-hidden font-sans transition-colors duration-300">
      ...
      <aside className="w-64 border-r border-black/5 dark:border-white/10 bg-white/40 dark:bg-zinc-950/40 backdrop-blur-xl flex flex-col z-20 shadow-lg">
        ...
        <div className="px-4 mt-6 mb-2 flex items-center justify-between">
          <p className="text-[10px] font-bold text-brand-500 dark:text-[var(--color-neon-purple)] uppercase tracking-widest px-3 flex items-center gap-1.5">
          ...
          {innovationItems.map((item) => {
            ...
            return (
              ...
              >
                {isActive && (
                  <motion.div
                    layoutId="active-innov"
                    className="absolute inset-0 bg-gradient-to-r from-purple-500/10 to-transparent dark:from-[var(--color-neon-purple)]/10 dark:to-transparent border-l-2 border-purple-500 dark:border-[var(--color-neon-purple)] z-0"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                  />
                )}
                <Icon className={cn("w-5 h-5 relative z-10", isActive ? "text-purple-500 dark:text-[var(--color-neon-purple)]" : "")} />
                <span className="relative z-10">{item.name}</span>
              </Link>
            );
          })}
        ...
        <div className="px-4 mt-6 mb-2 flex items-center justify-between">
          <p className="text-[10px] font-bold text-amber-500 dark:text-[var(--color-neon-gold)] uppercase tracking-widest px-3 flex items-center gap-1.5">
            ...
            return (
              <Link
                ...
                className={cn(
                  "flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all relative overflow-hidden group",
                  isActive
                    ? "text-brand-600 dark:text-white bg-white dark:bg-zinc-900/50 shadow-sm border border-amber-500/20 dark:border-[var(--color-neon-gold)]/20"
                    : "text-slate-600 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-zinc-200 hover:bg-black/5 dark:hover:bg-white/5"
                )}
              >
                {isActive && (
                  <motion.div
                    layoutId="active-nexus"
                    className="absolute inset-0 bg-gradient-to-r from-amber-500/10 to-transparent dark:from-[var(--color-neon-gold)]/10 dark:to-transparent border-l-2 border-amber-500 dark:border-[var(--color-neon-gold)] z-0"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                  />
                )}
                <Icon className={cn("w-5 h-5 relative z-10", isActive ? "text-amber-500 dark:text-[var(--color-neon-gold)]" : "")} />
                <span className="relative z-10">{item.name}</span>
              </Link>
            );
          })}
        ...
        <div className="p-4 border-t border-black/5 dark:border-white/10 flex flex-col gap-2 mt-auto">
          <button
