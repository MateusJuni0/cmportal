...
import { Users, ShieldAlert, Cpu, GitMerge, Fingerprint, MessageSquareWarning, Zap, CheckCircle2, Bot } from "lucide-react";
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      <div className="flex items-center justify-between">
        ...
        <div className="px-4 py-2 rounded-full border border-indigo-500/30 bg-indigo-500/10 text-indigo-500 text-xs font-bold tracking-widest uppercase flex items-center gap-2">
          ...
          <div className="flex justify-around mb-8 relative z-10">
            {personas.map((persona, i) => (
              <div key={i} className="flex flex-col items-center gap-2">
                <motion.div 
                  className="w-12 h-12 rounded-full border-2 border-indigo-500/50 flex items-center justify-center bg-black shadow-[0_0_15px_rgba(99,102,241,0.3)]"
                  animate={{ boxShadow: ["0 0 10px rgba(99,102,241,0.2)", "0 0 20px rgba(99,102,241,0.6)", "0 0 10px rgba(99,102,241,0.2)"] }}
          ...
          <div className="bg-black/50 border border-white/5 rounded-xl p-4 h-48 overflow-y-auto font-mono text-sm relative z-10">
            {boardLog.map((log, i) => (
              <motion.div 
                ...
                className={`mb-3 ${
                  log.includes("CFO") ? "text-emerald-500" : 
                  log.includes("CMO") ? "text-blue-500" : 
                  log.includes("Risk") ? "text-rose-500" : 
                  log.includes("Visionary") ? "text-amber-500" : 
                  "text-indigo-500"
                }`}
              >
                {">"} {log}
