...
import { 
  ...
  Cpu, History, Play, Send, Facebook, Instagram, Linkedin, Trash2 
} from "lucide-react";
...
export function CreativeLab() {
  ...
  const generateMutation = trpc.creative.generate.useMutation({
    onSuccess: (data) => {
      ...
      setRefinement("");
      addHistoryItem(data);
      addLog(`Ativo ${activeTab} sintetizado com sucesso pela IA.`, 'success');
    },
    onError: (err) => {
      setIsGenerating(false);
      addLog(`Erro na síntese: ${err.message}`, 'error');
    }
  });
  ...
  const handleGenerate = (isRefinement = false) => {
    ...
    addLog(`Enviando ordem de síntese para o motor de IA...`, 'info');
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          ...
          <GlassmorphismCard className="p-6 border border-white/5 bg-white/[0.01] flex-1 flex flex-col min-h-[300px]">
             <div className="flex justify-between items-center mb-6">
               ...
               {history.length > 0 && (
                 <button onClick={clearCreativeHistory} className="text-[8px] text-zinc-700 hover:text-red-500 uppercase font-black transition-colors">
                  ...
                  history.map((item) => (
                    <div key={item.id} className="p-4 bg-white/[0.02] border border-white/5 rounded-xl group hover:border-[var(--color-neon-purple)]/30 transition-all">
                       <div className="flex justify-between items-start mb-2">
                         <span className="text-[8px] text-zinc-600 font-black uppercase tracking-widest">{new Date(item.createdAt).toLocaleTimeString()} • {item.type}</span>
                         <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button className="text-zinc-500 hover:text-white" onClick={() => {
                              if (item.result) {
                    ...
                    {activeTab === "image" ? (
                      <>
                        ...
                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center backdrop-blur-sm">
                           <a href={result} download className="p-4 bg-white text-black rounded-full hover:scale-110 transition-transform shadow-2xl">
                      ...
                      <div className="p-8 font-mono text-sm text-zinc-300 whitespace-pre-wrap leading-relaxed h-full overflow-y-auto bg-[#0F0F0F] relative">
                         <div className="absolute top-6 right-6 p-2 bg-white/5 rounded-lg border border-white/10 text-zinc-500 hover:text-white cursor-pointer transition-colors" onClick={() => {
                           navigator.clipboard.writeText(result);
                           addLog("Copiado com sucesso", "success");
                         }}>
